...
func (f *ForwardingProvider) ForwardMessage(messageModel *models.MessageModel) error {
	...
	for _, cp := range cps {
		err := cp.SendMessage(messageModel)
		if err != nil {
			log.Err(err).Msg("Error occured while forwarding message")
